...
> **Welcome to Product-Track Week 3, Hotfix 1!**
...
1. **Read the comments** at the top of `searchService.py` - they describe the problem
...
| File | Purpose |
|------|---------|
| `searchService.py` | The code with bugs - **fix this file** |
...
### Bug #1
**What's wrong:** Should be len(arr) - 1

**How to find it:** Search for `BUG` in `searchService.py` - the comments around each bug explain what's broken.

### Bug #2
**What's wrong:** Should be left <= right (misses single element)
...
1. **Read the top comment block** in `searchService.py` carefully - it has:
